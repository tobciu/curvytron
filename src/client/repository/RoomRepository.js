...
class RoomRepository extends EventEmitter {
    ...
    join(name, password, callback) {
        ...
        this.client.addEvent('room:join', {name: name, password: password}, function (result) {
            if (result.success) {
                const clients = repository.createClients(result.clients);
                const master = clients.getById(result.master);
                const room = repository.createRoom(result.room, clients);
     ...
     * @param {Function} callback
     */
    setColor(player, color, callback) {
     ...
     * @param {Function} callback
     */
    setConfigMaxScore(maxScore, callback) {
     ...
     * @param {Function} callback
     */
    setConfigVariable(variable, value, callback) {
