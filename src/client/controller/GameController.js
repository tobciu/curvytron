...
class GameController extends AbstractController {
    constructor($scope, $routeParams, $location, client, repository, chat, radio, sound) {
        super($scope);
        ...
        const name = decodeURIComponent($routeParams.name);
     ...
     * @type {String}
     */
    confirmation = 'Are you sure you want to leave the game?';
    ...
    loadGame(game) {
        ...
        for (let avatar, i = this.game.avatars.items.length - 1; i >= 0; i--) {
    ...
    close() {
        if (this.game) {
            ...
            const avatars = this.game.avatars.filter(function () { return this.input; }).items;
