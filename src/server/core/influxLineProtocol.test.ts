...
describe('toLineProtocol', () => {
  ...
  it('quotes and escapes string field values', () => {
    ...
    expect(toLineProtocol('m', {}, { s: 'has "quotes"' })).toBe('m s="has \\"quotes\\""');
  });

  it('escapes commas, spaces and equals signs in measurement/tags', () => {
    expect(toLineProtocol('has space', {}, { v: 1 })).toBe('has\\ space v=1');
    expect(toLineProtocol('m', { 'a,b': 'c=d e' }, { v: 1 })).toBe('m,a\\,b=c\\=d\\ e v=1');
  });
});
